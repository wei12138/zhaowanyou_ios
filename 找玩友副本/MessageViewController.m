...
@interface MessageViewController ()<ASIHTTPRequestDelegate,AsynchronousRequestDelegate>
...
@implementation MessageViewController
{
    ...
    NSString* userID;
    MessageScro* message;
    AsynchronousRequest* asy; //网络请求解析器
}
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [views[indexPath.section] setImageWithURL:[[dataArray objectAtIndex:indexPath.section] valueForKey:@"photo"] placeholderImage:[UIImage imageNamed:@"gray_logo.png"]];
...
-(void) packegMessageToWeb
{
    ...
    asy.delegate = self;
    [asy getRequestForServes:path andPostValue:dic andIsJson:YES];
    //[self getMessageFromWeb:path and:dic];
}

-(void) AsyRequestFinished:(NSArray *)valueArray
{
    NSLog(@"获取的消息数据为%@",valueArray);
    asy.delegate = nil;
}
